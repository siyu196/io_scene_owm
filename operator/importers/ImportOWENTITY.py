...
from io_scene_owm.structures.EntitySettings import EntitySettings
...
class ImportOvertoolsEntity(bpy.types.Operator, ImportHelper):
    ...
    import_subentites: BoolProperty(
        name='Import Sub-Entities',
        description='Import non-root entities',
        default=True
    )

    def draw(self, context):
        column = self.layout
        column.label(text='Entity')
        column.prop(self, 'import_subentites')
    ...
    def execute(self, context):
        settings = EntitySettings(
            ...
            import_subentites=self.import_subentites
        )
        print(settings)
        return {'FINISHED'}
